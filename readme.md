...
This repository currently provides a careful local project skeleton: UV-based environment setup, configuration templates, shell helpers, Python runners, and starter modules for data preparation, downloads, SFT, GRPO, and evaluation. The evaluation runner now includes a reproducible reliability audit for uncertainty and perturbation stability.
...
## Reliability Audit For Task 4-5

The `eval` runner now supports a simple but fully local reliability workflow for the assignment items:

- uncertainty estimation: normalized entropy, top-1 margin, calibration error (ECE), and bucketed error analysis;
- stability audit: perturb text with controlled rule-based transformations and measure top-1 flip-rate plus top-k consistency;
- reproducible artifacts: JSON summaries, JSONL per-example outputs, saved perturbation examples, and plots when `matplotlib` is installed.

Default mode trains a baseline ranker on the train split and evaluates the test split:
...
- `model.source`: `baseline` to fit TF-IDF + multinomial logistic regression, or `predictions` to audit an existing `predictions.jsonl`.
